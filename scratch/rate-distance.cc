...
main(int argc, char* argv[])
{
    ...
    std::string staManager{"ns3::MinstrelHtWifiManager"}; //Side Note: The standard based on the minstrel algorithm for High Throughput
    std::string apManager{"ns3::MinstrelHtWifiManager"};
    std::string standard{"802.11a"}; //Thee wifi standard
    std::string outputFileName{"interference-3nodes-all1"};
    ...
    interfererMac.SetType("ns3::ApWifiMac", "Ssid", SsidValue(ssid));
    ...
    interfererAlloc->Add(Vector(0.0, 0.0, 0.0)); // Adjust as needed
    ...
    // Interferer Setup
    OnOffHelper interfererApp("ns3::UdpSocketFactory", InetSocketAddress("255.255.255.255", 9));
    interfererApp.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
    interfererApp.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0]"));
    interfererApp.SetAttribute("DataRate", StringValue("1000Mbps")); // Tune this as needed
    interfererApp.SetAttribute("PacketSize", UintegerValue(4096));

...
NS_LOG_COMPONENT_DEFINE("TARAWifiManager");

namespace ns3
{

/// TARAWifiRemoteStation structure
struct TARAWifiRemoteStation : MinstrelWifiRemoteStation
{
...
NS_OBJECT_ENSURE_REGISTERED(TARAWifiManager);

TypeId
TARAWifiManager::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::TARAWifiManager")
            .SetParent<WifiRemoteStationManager>()
            .AddConstructor<TARAWifiManager>()
            .SetGroupName("Wifi")
            .AddAttribute("UpdateStatistics",
                          "The interval between updating statistics table",
                          TimeValue(MilliSeconds(50)),
                          MakeTimeAccessor(&TARAWifiManager::m_updateStats),
                          MakeTimeChecker())
            .AddAttribute("LegacyUpdateStatistics",
                          "The interval between updating statistics table (for legacy Minstrel)",
                          TimeValue(MilliSeconds(100)),
                          MakeTimeAccessor(&TARAWifiManager::m_legacyUpdateStats),
                          MakeTimeChecker())
            .AddAttribute("LookAroundRate",
                          "The percentage to try other rates (for legacy Minstrel)",
                          UintegerValue(10),
                          MakeUintegerAccessor(&TARAWifiManager::m_lookAroundRate),
                          MakeUintegerChecker<uint8_t>(0, 100))
            .AddAttribute("EWMA",
                          "EWMA level",
                          UintegerValue(75),
                          MakeUintegerAccessor(&TARAWifiManager::m_ewmaLevel),
                          MakeUintegerChecker<uint8_t>(0, 100))
            .AddAttribute("SampleColumn",
                          "The number of columns used for sampling",
                          UintegerValue(10),
                          MakeUintegerAccessor(&TARAWifiManager::m_nSampleCol),
                          MakeUintegerChecker<uint8_t>())
            .AddAttribute("PacketLength",
                          "The packet length used for calculating mode TxTime (bytes)",
                          UintegerValue(1200),
                          MakeUintegerAccessor(&TARAWifiManager::m_frameLength),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("UseLatestAmendmentOnly",
                          "Use only the latest amendment when it is supported by both peers",
                          BooleanValue(true),
                          MakeBooleanAccessor(&TARAWifiManager::m_useLatestAmendmentOnly),
                          MakeBooleanChecker())
            .AddAttribute("PrintStats",
                          "Control the printing of the statistics table",
                          BooleanValue(false),
                          MakeBooleanAccessor(&TARAWifiManager::m_printStats),
                          MakeBooleanChecker())
            .AddAttribute("TARARate",
                          "The TARA Algorithm current MCS prediction (MCS id)",
                          UintegerValue(0),
                          MakeUintegerAccessor(&TARAWifiManager::m_TARARate),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("TARASnr",
                          "The TARA Algorithm current SNR prediction (MCS id)",
                          DoubleValue(0),
                          MakeDoubleAccessor(&TARAWifiManager::m_TARASnr),
                          MakeDoubleChecker<double>())
            .AddAttribute("BerThreshold",
                          "The maximum Bit Error Rate acceptable at any transmission mode",
                          DoubleValue(1e-6),
                          MakeDoubleAccessor(&TARAWifiManager::m_ber),
                          MakeDoubleChecker<double>())
            .AddTraceSource("Rate",
                            "Traced value for rate changes (b/s)",
                            MakeTraceSourceAccessor(&TARAWifiManager::m_currentRate),
                            "ns3::TracedValueCallback::Uint64");
    return tid;
}

TARAWifiManager::TARAWifiManager()
    : m_numGroups(0),
      m_numRates(0),
      m_currentRate(0)
{
...
int64_t
TARAWifiManager::AssignStreams(int64_t stream)
{
...
void
TARAWifiManager::SetupPhy(const Ptr<WifiPhy> phy)
{
...
void
TARAWifiManager::SetupMac(const Ptr<WifiMac> mac)
{
...
uint16_t
TARAWifiManager::GetChannelWidthForNonHtMode(WifiMode mode) const
{
...
void
TARAWifiManager::BuildSnrThresholds()
{
    m_thresholds.clear();
    WifiMode mode;
...
double
TARAWifiManager::GetSnrThreshold(WifiTxVector txVector)
{
...
void
TARAWifiManager::AddSnrThreshold(WifiTxVector txVector, double snr)
{
...
bool
TARAWifiManager::IsValidMcs(Ptr<WifiPhy> phy,
                                  uint8_t streams,
                                  uint16_t chWidth,
                                  WifiMode mode)
{
...
Time
TARAWifiManager::CalculateMpduTxDuration(Ptr<WifiPhy> phy,
                                               uint8_t streams,
                                               uint16_t gi,
                                               uint16_t chWidth,
                                               WifiMode mode,
...
Time
TARAWifiManager::GetFirstMpduTxTime(uint8_t groupId, WifiMode mode) const
{
...
void
TARAWifiManager::AddFirstMpduTxTime(uint8_t groupId, WifiMode mode, Time t)
{
...
Time
TARAWifiManager::GetMpduTxTime(uint8_t groupId, WifiMode mode) const
{
    NS_LOG_FUNCTION(this << +groupId << mode);
...
void
TARAWifiManager::AddMpduTxTime(uint8_t groupId, WifiMode mode, Time t)
{
    NS_LOG_FUNCTION(this << +groupId << mode << t);
...
WifiRemoteStation*
TARAWifiManager::DoCreateStation() const
{
    NS_LOG_FUNCTION(this);
    TARAWifiRemoteStation* station = new TARAWifiRemoteStation();
...
void
TARAWifiManager::CheckInit(TARAWifiRemoteStation* station)
{
...
void
TARAWifiManager::DoReportRxOk(WifiRemoteStation* st, double rxSnr, WifiMode txMode)
{
    NS_LOG_FUNCTION(this << st);
    NS_LOG_DEBUG(
        "DoReportRxOk m_txrate=" << static_cast<TARAWifiRemoteStation*>(st)->m_txrate);
}

void
TARAWifiManager::DoReportRtsFailed(WifiRemoteStation* st)
{
    NS_LOG_FUNCTION(this << st);
    TARAWifiRemoteStation* station = static_cast<TARAWifiRemoteStation*>(st);
    CheckInit(station);
    if (!station->m_initialized)
    {
        return;
...
void
TARAWifiManager::DoReportRtsOk(WifiRemoteStation* st,
                                     double ctsSnr,
                                     WifiMode ctsMode,
                                     double rtsSnr)
{
...
void
TARAWifiManager::DoReportFinalRtsFailed(WifiRemoteStation* st)
{
    NS_LOG_FUNCTION(this << st);
    TARAWifiRemoteStation* station = static_cast<TARAWifiRemoteStation*>(st);
...
void
TARAWifiManager::DoReportDataFailed(WifiRemoteStation* st)
{
    NS_LOG_FUNCTION(this << st);
    TARAWifiRemoteStation* station = static_cast<TARAWifiRemoteStation*>(st);
...
void
TARAWifiManager::DoReportDataOk(WifiRemoteStation* st,
                                      double ackSnr,
                                      WifiMode ackMode,
                                      double dataSnr,
                                      ...
                                      uint8_t dataNss)
{
    //NS_LOG_UNCOND("Entrei na ReportDataOk");
...
void
TARAWifiManager::DoReportFinalDataFailed(WifiRemoteStation* st)
{
    NS_LOG_FUNCTION(this << st);
    TARAWifiRemoteStation* station = static_cast<TARAWifiRemoteStation*>(st);

    //NS_LOG_UNCOND("Entrei na FinalDataFailed");
...
void
TARAWifiManager::DoReportAmpduTxStatus(WifiRemoteStation* st,
                                             ...
                                             uint8_t dataNss)
{
    ...
    //NS_LOG_UNCOND("Entrei na AmpduTxStatus");
...
void
TARAWifiManager::UpdateRate(TARAWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
    ...
    if (!station->m_isSampling)
    {
        ...
        // ----------------- TARA-config ----------------- 
        if (station ->m_longRetry < 2){
            station->m_txrate = m_TARARate;
        }
        // ----------------- TARA-config ----------------- 
    ...
    else
    {
        ...
        // ----------------- TARA-config ----------------- 
        if (station->m_longRetry < 1 + 2)
        {
            NS_LOG_DEBUG("Sampling use the TARA rate");
            station->m_txrate = m_TARARate;
        }
        // ----------------- TARA-config ----------------- 
        else if (station->m_longRetry <
...
void
TARAWifiManager::UpdateRetry(TARAWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
    station->m_shortRetry = 0;
    station->m_longRetry = 0;
...
void
TARAWifiManager::UpdatePacketCounters(TARAWifiRemoteStation* station,
                                            uint16_t nSuccessfulMpdus,
                                            uint16_t nFailedMpdus)
{
...
uint16_t
TARAWifiManager::UpdateRateAfterAllowedWidth(uint16_t txRate, uint16_t allowedWidth)
{
...
WifiTxVector
TARAWifiManager::DoGetDataTxVector(WifiRemoteStation* st, uint16_t allowedWidth)
{
...
WifiTxVector
TARAWifiManager::DoGetRtsTxVector(WifiRemoteStation* st)
{
    NS_LOG_FUNCTION(this << st);
    TARAWifiRemoteStation* station = static_cast<TARAWifiRemoteStation*>(st);
...
bool
TARAWifiManager::DoNeedRetransmission(WifiRemoteStation* st,
...
uint32_t
TARAWifiManager::CountRetries(TARAWifiRemoteStation* station)
{
    uint8_t maxProbRateId = GetRateId(station->m_maxProbRate);
    uint8_t maxProbGroupId = GetGroupId(station->m_maxProbRate);
    uint8_t maxTpRateId = GetRateId(station->m_maxTpRate);
...
uint16_t
TARAWifiManager::GetNextSample(TARAWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
void
TARAWifiManager::SetNextSample(TARAWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
    do
    {
...
uint16_t
TARAWifiManager::FindRate(TARAWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
void
TARAWifiManager::UpdateStats(TARAWifiRemoteStation* station)
{
    //NS_LOG_UNCOND("Entrei na UpdateStats");
    NS_LOG_FUNCTION(this << station);
    ...
    // ----------------- TARA-config -----------------     
    //inspired from IdealWifiManager

    WifiTxVector txVector;
    //txVector.SetChannelWidth(GetChannelWidth(station));
    txVector.SetChannelWidth(20);
    uint16_t guardInterval = static_cast<uint16_t>(
    std::max(GetShortGuardIntervalSupported(station) ? 400 : 800,
                GetShortGuardIntervalSupported() ? 400 : 800));
    txVector.SetGuardInterval(guardInterval);
    txVector.SetNss(1);
    
    // NS_LOG_UNCOND(" Current TARARate: " << m_TARARate << " | Current MaxTP: " << station->m_maxTpRate << " | Current MaxTP2: " << station->m_maxTpRate2 << " | Current MaxProb: " << station->m_maxProbRate);
    
    double currentSnr = pow(10, m_TARASnr / 10);
    double candidateThreshold;
    uint16_t newRateId = 0;
    uint64_t TARARate = 0;
    for (uint8_t i = 0; i < GetNMcsSupported(station); i++)
    {
        WifiMode mode = GetMcsSupported(station, i);
        txVector.SetMode(mode);
        uint64_t candidateDataRate = mode.GetDataRate(txVector.GetChannelWidth(),
                                                txVector.GetGuardInterval(),
                                                1); // nss = 1

        candidateThreshold = GetSnrThreshold(txVector);
        //NS_LOG_UNCOND("Current SNR: " << currentSnr << " | CandidateThreshold: " << candidateThreshold);
        //NS_LOG_UNCOND("Current Rate: " << TARARate << " | CandidateRate: " << candidateDataRate);
        
        if (currentSnr > candidateThreshold && candidateDataRate > TARARate)
        {
            //NS_LOG_UNCOND("Entrei");
            TARARate = candidateDataRate;
            newRateId = i;   
        }
    }
    
    if(newRateId != m_TARARate)
    {
        NS_LOG_UNCOND("Old TARARate: " << m_TARARate << " | New TARARate: " << newRateId);
        NS_LOG_UNCOND("Current SNR: " << currentSnr << " | Threshold of rate_ " << newRateId << ": " << candidateThreshold);
        m_TARARate = newRateId;
    }

    //NS_LOG_UNCOND("New TARARate: " << m_TARARate);
    //After updating Station Th Rates, MaxTP becomes MaxTP2, MaxTP2 is no longer used... and the new MaxTP comes from TARA
    
    if(m_TARARate > station->m_maxTpRate)
    {
        station->m_maxTpRate2 = station->m_maxTpRate;
        station->m_maxTpRate = m_TARARate;    
    }
    //station->m_maxTpRate2 = station->m_maxTpRate;
    //station->m_maxTpRate = m_TARARate;
    ...
    //station->m_maxTpRate2 = station->m_maxTpRate;
    //station->m_maxProbRate = station->m_maxTpRate;
...
double
TARAWifiManager::CalculateThroughput(TARAWifiRemoteStation* station,
                                           uint8_t groupId,
                                           uint8_t rateId,
                                           double ewmaProb)
{
...
void
TARAWifiManager::SetBestProbabilityRate(TARAWifiRemoteStation* station, uint16_t index)
{
    GroupInfo* group;
    TARARateInfo rate;
    uint8_t tmpGroupId;
 ...
 */
void
TARAWifiManager::SetBestStationThRates(TARAWifiRemoteStation* station, uint16_t index)
{
    uint8_t groupId;
    uint8_t rateId;
    double th;
    ...
    if (th > maxTpTh || (th == maxTpTh && prob > maxTpProb))
    {
        ...
        //NS_LOG_UNCOND("New Station MaxTP rate: " << station->m_maxTpRate);
    }
    else if (th > maxTp2Th || (th == maxTp2Th && prob > maxTp2Prob))
    {
        station->m_maxTpRate2 = index;
        //NS_LOG_UNCOND("New Station MaxTP2 rate: " << station->m_maxTpRate2);
    }
    ...
    if (th > maxTpTh || (th == maxTpTh && prob > maxTpProb))
    {
        ...
        //NS_LOG_UNCOND("New Group MaxTP rate: " << station->m_maxTpRate);
    }
    else if (th > maxTp2Th || (th == maxTp2Th && prob > maxTp2Prob))
    {
        group->m_maxTpRate2 = index;
        //NS_LOG_UNCOND("New Group MaxTP2 rate: " << station->m_maxTpRate2);
    }
}

void
TARAWifiManager::RateInit(TARAWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
    ...
    for (uint8_t groupId = 0; groupId < m_numGroups; groupId++)
    {
        if (m_minstrelGroups[groupId].isSupported)
        {
            ...
            station->m_groupsTable[groupId].m_ratesTable =
                TARARate(m_numRates); /// Create the rate list for the group.
            for (uint8_t i = 0; i < m_numRates; i++)
            {
...
void
TARAWifiManager::CalculateRetransmits(TARAWifiRemoteStation* station, uint16_t index)
{
...
void
TARAWifiManager::CalculateRetransmits(TARAWifiRemoteStation* station,
                                            uint8_t groupId,
                                            uint8_t rateId)
{
...
double
TARAWifiManager::CalculateEwmsd(double oldEwmsd,
                                      double currentProb,
                                      double ewmaProb,
                                      double weight)
{
...
void
TARAWifiManager::InitSampleTable(TARAWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
void
TARAWifiManager::PrintTable(TARAWifiRemoteStation* station)
{
...
void
TARAWifiManager::StatsDump(TARAWifiRemoteStation* station,
                                 uint8_t groupId,
                                 std::ofstream& of)
{
...
uint16_t
TARAWifiManager::GetIndex(uint8_t groupId, uint8_t rateId)
{
...
uint8_t
TARAWifiManager::GetRateId(uint16_t index)
{
...
uint8_t
TARAWifiManager::GetGroupId(uint16_t index)
{
    NS_LOG_FUNCTION(this << index);
...
uint8_t
TARAWifiManager::GetHtGroupId(uint8_t txstreams, uint16_t gi, uint16_t chWidth)
{
...
uint8_t
TARAWifiManager::GetVhtGroupId(uint8_t txstreams, uint16_t gi, uint16_t chWidth)
{
    NS_LOG_FUNCTION(this << +txstreams << gi << chWidth);
    uint8_t giIndex = (gi == 400) ? 1 : 0;
    uint8_t widthIndex;
...
uint8_t
TARAWifiManager::GetHeGroupId(uint8_t txstreams, uint16_t gi, uint16_t chWidth)
{
    NS_LOG_FUNCTION(this << +txstreams << gi << chWidth);
    uint8_t giIndex;
    if (gi == 800)
...
uint16_t
TARAWifiManager::GetLowestIndex(TARAWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
uint16_t
TARAWifiManager::GetLowestIndex(TARAWifiRemoteStation* station, uint8_t groupId)
{
...
WifiModeList
TARAWifiManager::GetHeDeviceMcsList() const
{
...
WifiModeList
TARAWifiManager::GetVhtDeviceMcsList() const
{
...
WifiModeList
TARAWifiManager::GetHtDeviceMcsList() const
{
